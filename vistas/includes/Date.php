...
class Zebra_Form_Date extends Zebra_Form_Control
{
    ...
    function toHTML()
    {
        ...
        // all date controls should have the "date" rule set or we trigger an error
        if (!isset($this->rules['date'])) trigger_error('The <strong>' . $this->form_properties['name'] . '</strong> form\'s <strong>' . $attributes['name'] . '</strong> control <strong>requires</strong> the <em>date</em> rule to be set', E_USER_ERROR);
