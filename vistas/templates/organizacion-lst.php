...
if (isset($_GET["error"]) && $_GET["error"] == 1) {
    if ($_GET["tipo_error"] == 'error_eliminar') {
        $error = $label["error_eliminar"] . $label["organizacion"] . $label['seleccionado'];
...
echo Utils::getDivTitulo($label["listado_organizacion"]);

$casoUso = "organizacion";
$template = "lst";
$hrefEdit = '?frm=' . $casoUso . '&amp;template=' . $template;
$action = "../negocio/crud_" . $casoUso . ".php";
$myHtml = new lst($casoUso, 'Organizaciones', $label, $_GET, $hrefEdit, "", "", $action, $mySession);
$myHtml->getLst();
?>