...
if (isset($_GET["id"]) && ($_GET["id"] == 0)) {
    echo Utils::getDivTitulo($label["nuevo_estadocivil"]);
    //cargar los datos a modificar    
    $id = 0;
    $estado_civil = '';
    $MM_from = 'guardar_nuevo';
} else {
    echo Utils::getDivTitulo($label["modificar_estadocivil"]);
    //cargar los datos a modificar
    $MyEstadoCivil = new Estadocivil();
    $MyEstadoCivil->getRecord($_GET["id"]);
    $aDatos = $MyEstadoCivil->objConexion->crearArreglo();
...
$form = new Zebra_Form('form', 'POST', '../negocio/crud_estadocivil.php');
...
$obj = & $form->add('text', 'estado_civil', $estado_civil);
