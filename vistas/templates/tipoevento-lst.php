...
if (isset($_GET["error"]) && $_GET["error"] == 1) {
    if ($_GET["tipo_error"] == 'error_eliminar') {
        $error = $label["error_eliminar"] . $label["error_tipo_evento"] . $label['seleccionado'];
...
echo Utils::getDivTitulo($label["tipo_evento"]);
$casoUso = "tipoevento";
$template = "lst";
$hrefEdit = '?frm=' . $casoUso . '&amp;template=' . $template;
$action = "../negocio/crud_" . $casoUso . ".php";
$myHtml = new lst($casoUso, 'tipoevento', $label, $_GET, $hrefEdit, "", "", $action,$mySession);
$myHtml->getLst();
?>