...
if (isset($_GET["error"]) && $_GET["error"] == 1) {
    if ($_GET["tipo_error"] == 'error_eliminar') {
        $error = $label["error_eliminar"] . $label["tipo_servicio"] . $label['seleccionado'];
...
echo Utils::getDivTitulo($label["listar_tipo_servicio"]);
$casoUso = "tiposervicio";
$template = "lst";
$hrefEdit = '?frm=' . $casoUso . '&amp;template=' . $template;
$action = "../negocio/crud_" . $casoUso . ".php";
$myHtml = new lst($casoUso, 'tiposervicio', $label, $_GET, $hrefEdit, "", "", $action,$mySession);
$myHtml->getLst();
?>