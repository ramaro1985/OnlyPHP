...
if (isset($_GET["error"]) && $_GET["error"] == 1) {
    if ($_GET["tipo_error"] == 'error_eliminar') {
        $error = $label["error_eliminar"] . $label["dialecto"] . $label['seleccionado'];
...
echo Utils::getDivTitulo($label["listado_dialecto"]);
$casoUso = "dialecto";
$template = "lst";
$hrefEdit = '?frm=' . $casoUso . '&amp;template=' . $template;
$action = "../negocio/crud_" . $casoUso . ".php";
$myHtml = new lst($casoUso, 'v_dialecto', $label, $_GET, $hrefEdit, "", "", $action,$mySession);
$myHtml->getLst();
?>