...
if (isset($_GET["error"]) && $_GET["error"] == 1) {
        if ($_GET["tipo_error"] == 'error_eliminar') {
            $error = $label["error_eliminar"] . $label["tipoevaluacion"].$label['seleccionado'];
...
echo Utils::getDivTitulo( $label["listado_tipoevaluacion"] );

$casoUso = "tipoevaluacion";
$template = "lst";
$hrefEdit = '?frm=' . $casoUso . '&amp;template=' . $template;
$action = "../negocio/crud_" . $casoUso . ".php";
$myHtml = new lst($casoUso, 'Tipoevaluacion', $label, $_GET, $hrefEdit, "", "", $action, $mySession);
$myHtml->getLst();
?>